...
import { useGetCategoryById, useGetTopicById } from '../../hook';
...
function CategoryPage() {
  ...
  const category = useGetCategoryById(categoryId, 20, 'createdAt', 'DESC');
  const topSongs = useGetCategoryById(categoryId, 15, 'views', 'DESC');
  return (
    <>
      <div className="relative -mx-[59px] pt-[30%]">
        {category.data?.data.image && (
          <img
            className="absolute top-0 h-full w-full object-cover"
            src={`${import.meta.env.VITE_API_FILE_URL}/${category.data?.data.image}`}
            alt=""
          />
        )}
      </div>
      <div className="mt-[48px]">
        <ListSongs title={'Mới nhất'} songs={category.data?.data.songInfo.slice(0, 5)} navigate={false} />
      </div>
      <div className="mt-[48px]">
